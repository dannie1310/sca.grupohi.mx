...
<tr><td colspan="10">&nbsp;</td></tr>
<tr><td colspan="10" align="right"><i>Fecha de Corte:<?php echo $_REQUEST['final'];?></i></td></tr>
<tr><td colspan="10" align="right"><i>Fecha:<?php echo date("d-m-Y");?></i></td></tr>
<?php
$sql = "select distinct IdBloque from topografias where Fecha<='".$final."' order by IdBloque";
...
while($vsql = $SCA->fetch($rsql)){
    ...
    <tr style="background-color:#333; color:#FFF; font-weight:bold;"><td colspan="10"><?php echo $SCA->regresaDatos2('bloques','Descripcion','IdBloque', $vsql['IdBloque']) ?></td></tr>
    	<?php
		$sql1 = "select distinct IdMaterial from topografias where Fecha<='".$final."' AND IdBloque='".$vsql['IdBloque']."' order by IdMaterial";
		...
		while($vsql1 = $SCA->fetch($rsql1)){
            ...
            <?php
			$sql2 = "select * from topografias where Fecha<='".$final."' AND IdBloque='".$vsql['IdBloque']."' AND IdMaterial='".$vsql1['IdMaterial']."' ORDER BY Fecha";
