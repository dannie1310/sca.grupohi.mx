...
WHERE o.IdOrigen = $IdOrigen AND t.IdTiro = $IdTiro AND fecha = '".$fecha."'";
	//$SQL = "select * from vw_cronentvia where fecha BETWEEN '".$fi_fi."' AND '".$ff_fi."';";
	...
	while($row = mysql_fetch_array($result))
	{
	  $contador_viajes = $contador_viajes + 1;
		...
		IF($viajes[3] 	[$contador_viajes] == $viajes[3] 	[$contador_viajes-1])
		...
		$viajes[14] [$contador_viajes] 		= $row[TiempoMinimo];
	}
	//echo 'resultados='.$contador_viajes;
	//mysql_close($link);
  ...
  <!--<tr>
    <td colspan="9" class="tituloreporte"><span class="titulo">Consulta de Lapsos entre Viajes por Destino</span></td>
  </tr>
  <tr>
  ...
  >
    ...
    <td align="center"><?php $qcrono = "SELECT TiempoMinimo FROM cronometrias WHERE IdRuta = $IdRuta";
					echo $crono;
