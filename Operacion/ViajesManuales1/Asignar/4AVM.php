		...
		//Validamos el Numero de Camiones a Procesar
			...
			for($a=1;$a<=$NumVM;$a++)
			{
				...
				for($b=1;$b<=$NVMS[$a];$b++)
				{
					...
					$values=" VALUES('".$Hoy."', '".$Hora."', ".$_SESSION[Proyecto].", ".$Economicos[$a].", ".$Origenes[$a].", '".$Fechas[$a]."', '".$Horav[$a]."', ".$Destinos[$a].", '".$Fechas[$a]."', '".$Horav[$a]."', ".$Materiales[$a].", '".$Observaciones[$a]."', '".$Creo."', 29);";
					$sql=$insert.$values;
					//echo "<br>".$sql;
...
$asunto = "Registro de Viajes Manuales  "."(".date("d-m-Y H:i:s").")"; 
