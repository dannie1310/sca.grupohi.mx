...
<thead>
  <tr>
    ...
    <th rowspan="2">Cami&oacute;n<br />'.$sca->regresaSelectBasicoRet("camiones","IdCamion","Economico","Estatus=1","asc","1","","camionp","camion".$i."",'class="camiones_p" contador="'.$i.'"').'</th>
    <th rowspan="2">Cubicaci&oacute;n</th>
       <th rowspan="2">Origen<br />'.$sca->regresaSelectBasicoRet("origenes","IdOrigen","Descripcion","Estatus=1","asc","1","","origenp","origenp",'class="calcula_ruta_p"').'</th>
    <th rowspan="2">Tiro<br />'.$sca->regresaSelectBasicoRet("tiros","IdTiro","Descripcion","Estatus=1","asc","1","","tirop","tirop",'class="calcula_ruta_p"').'</th>
    <th rowspan="2">Ruta</th>
    <th rowspan="2">Material<br />'.$sca->regresaSelectBasicoRet("materiales","IdMaterial","Descripcion","Estatus=1","asc","1","","materialp","materialp",'class="devuelve_tarifa_p" ').'</th>
  ...
  $salida.='
	...
	function registra_viajes($frm)
	{
		...
		for($x=0;$x<$i;$x++)
		{
			...
			{
				$SQLs="call registra_viajes_netos_viajes(".$_SESSION["Proyecto"].",'".fechasql($frm["fecha"][$x])."',".$frm["camion"][$x].",".$frm["cubicacion"][$x].",".$frm["origen"][$x].",".$frm["tiro"][$x].",".$frm["ruta"][$x].",".$frm["material"][$x].",".$frm["primerkm"][$x].",".$frm["kmsubsecuentes"][$x].",".$frm["kmadicionales"][$x].",'".$frm["turno".$x]."','".$frm["observaciones"][$x]."',".$_SESSION["IdUsuarioAc"].",@OK)";
				$r=$sca->consultar($SQLs);
...
select{ width:114px;}
select.camiones,select.camiones_p,select.ruta{ width:50px;}
...
table.resultados tbody tr.green td{ color:#090;}
table.resultados tbody tr.red td{ color:#F00;}
...
$(function() {
													...
													$.ajax(
														   {
																...
																success:function(data){
																	$("#tab").html(data);
																						...
																						if(data.kind=="green")
																						{
																							$("div#tabla").load("jq/jq_muestra_turno_registrado.php?id_turno="+data.id);
																							$("div.instrucciones").html("Si lo desea puede agregar un horario, indicando la hora de inicio y de fin.");
																						}*/
																					  },
																error:function(){
																					//ADV.setAdv({kind:"red",msg:"Error al requerir la página de registro de turnos"});
																				}
              }
														   )
	...
	$('.camiones_p').live("change",function(){ c=$(this).attr("value");
											   $('.camiones').each(function()
																			{
																				$('.camiones').attr("value",c);
																				i=$(this).attr("contador"); 
																				xajax_cubicacion(c,i);
																			})  
															
											});
	$('.calcula_ruta_p').live("change",function(){
												 vorigen=$("#origenp").attr("value");
												 vtiro=$("#tirop").attr("value");
												 $('.calcula_ruta').each(function()
																			{
																			i=$(this).attr("contador");
																			  if($(this).attr("id")=="origen"+i)
																			  		$(this).attr("value",vorigen);
																			   if($(this).attr("id")=="tiro"+i)
																			   		$(this).attr("value",vtiro);
	...
	$('.devuelve_tarifa_p').live("change",function(){ vmaterial=$(this).attr("value");
											   $('.devuelve_tarifa').each(function()
																			{
																				i=$(this).attr("contador");
																				$(this).attr("value",vmaterial);
																				xajax_tarifa(vmaterial,i);
																			})  ;
...
<thead>
  <tr>
    <th rowspan="2">#</th>
    <th rowspan="2" style="width:116px">Fecha</th>
    <th rowspan="2">Cami&oacute;n<br /><?php echo $sca->regresaSelectBasicoRet("camiones","IdCamion","Economico","Estatus=1","asc","1","","camionp","camionp",'class="camiones_p"'); ?></th>
    <th rowspan="2">Cubicaci&oacute;n</th>
    <th rowspan="2">Origen<br /><?php echo $sca->regresaSelectBasicoRet("origenes","IdOrigen","Descripcion","Estatus=1","asc","1","","origenp","origenp",'class="calcula_ruta_p origen"'); ?></th>
    <th rowspan="2">Tiro<br /><?php echo $sca->regresaSelectBasicoRet("tiros","IdTiro","Descripcion","Estatus=1","asc","1","","tirop","tirop",'class="calcula_ruta_p tiro"'); ?></th>
    <th rowspan="2">Ruta</th>
    <th rowspan="2">Material<br /><?php echo $sca->regresaSelectBasicoRet("materiales","IdMaterial","Descripcion","Estatus=1","asc","1","","materialp","materialp",'class="devuelve_tarifa_p" '); ?></th>
    <th colspan="3">Tarifas</th>
    <th rowspan="2">Turno</th>
    <th rowspan="2">Observaciones</th>
  </tr>
