	 			...
	 			while($vfolio = mysql_fetch_array($result_folio)){
                    ...
                    <table class="reporte" width="804px;" style="margin-left:10px; font-weight:600;">
                    	<tr>
                            ...
                            <td align="left">
                            <?php 
                            if($vfolio["FirmadoPDF"]>0){ ?>
			                    <a href="firma_concliliacion/<?= $_SESSION[ProyectoGlobal].'-'.$vfolio["idconciliacion"]?>.pdf" download>
			                    	<img src="../../Imagenes/pdf.gif" idconciliacion="<?php echo $vfolio["idconciliacion"];?>" idsindicato="<?php echo $vfolio["idsindicato"];?>" fecha_inicial="<?php echo $vfolio["fecha_inicial"];?>" fecha_final="<?php echo $vfolio["fecha_final"];?>" class="firma_pdf" alt="Conciliaci&oacute;n firmada" title="Conciliaci&oacute;n firmada"/>
			                    </a>

			                    <img class="eliminar_firma" idconciliacion="<?php echo $vfolio["idconciliacion"];?>"  src="../../Imagenes/rechazado.gif"  alt="Eliminar Conciliaci&oacute;n firmada" title="Eliminar Conciliaci&oacute;n firmada"/>
			                    <?php 
			                }else { ?>
		                         ...
		                         if($vfolio["estado"]==1){ ?>
									...
									?>"/>
				                        ...
				                        <a href="#" 
				                        class="open" 
				                        idconciliacion="<?= $vfolio['idconciliacion'] ?>"
				                        fechainicio="<?= $vfolio["fecha_inicial"] ?>"
				                        fechafin="<?= $vfolio["fecha_final"] ?>">
				                        	<img 
				                        		src="../../Imagenes/upload.png" 
				                        		alt="Subir firma del la conciliacion en pdf" 
				                        		Title="Subir firma del la conciliacion en pdf" 
				                        	></a>
...
<div id="dialog"></div>
<div id="mensajes"></div>
<div id="fileform" title="Subiendo un pdf de la conciliacion firmada">
	<form enctype="multipart/form-data" class="formulariofile" method="post">
		<table class="reporte" width="300px">
		    <tr>
		    	<td colspan="2" >
		    		<input type="file"  name="archivo" idruta="archivo" accept="application/pdf"/>
		    	</td>
		       
			</tr>
			<tr>
				 <td ><input type="submit"   value="Subir pdf"  style="width:150px">
				 </td>
				<td >
					<input type="button"  value="Cancelar" style="width:150px">
					<input type="hidden" name="idconcliacion" id="idconcliacion" value="">
...
$(function() {
    ...
    $( '.formulariofile input[type=button]').on('click', function(){
    	$('#fileform').dialog('close');
    });
    $( '.eliminar_firma').on('click', function(){
    	$.post("eliminar_firma.php",{idconcliacion:$(this).attr('idconciliacion')},function(data){
			console.log(data);
			location.reload();
		});
    });
    ...
    $( '.formulariofile' )
 		.submit( function( e ) {
    	var formData = new FormData($(".formulariofile")[0]);
    	if(!$('.formulariofile input[type=file]').val()){
    		if(confirm('Por favor selecione un archivo en formato PDF')){
    			$('.formulariofile input[type=file]').trigger("click");
    		}
    		return false;
    	}
    	
    	 $('.formulariofile input').attr('disabled','disabled');
    	$.ajax({
            url: 'upload.php',  
            type: 'POST',
            data: formData,
            ...
            processData: false,
            beforeSend: function(){
                message = $("<span class='before'>Cargando firma, por favor espere...</span>");
                showMessage(message);

            },
            //una vez finalizado correctamente
            success: function(data){
                message = $("<span class='success'>El archivo pdf ha cargando correctamente.</span>");
             	showMessage(message);
             	//$('#fileform').dialog('close');
                $('.formulariofile input').removeAttr('disabled');                
				$('.formulariofile input[type=file]').val('');	
				setTimeout(function(){
					showMessage('');
					location.reload();
				 }, 2000)
            ...
            error: function(){
                message = $("<span class='error'>Ha ocurrido un error, intente otra vez</span>");
                showMessage(message);
                  $('.formulariofile input').removeAttr('disabled');
            }
        });
    ...
    dialog = $( "#fileform" ).dialog({
      autoOpen: false,
      height: 150,
      width: 350,
      modal: true,
      beforeClose: function (event, ui) {
      	$('.formulariofile idconcliacion').val('');
		$('.formulariofile fechainicio').val('');
		$('.formulariofile fechafin').val('');
      }

    });

	$('.open').click(function() {
		$('.formulariofile #idconcliacion').val($(this).attr('idconciliacion'));
		$('.formulariofile #fechainicio').val($(this).attr('fechainicio'));
		$('.formulariofile #fechafin').val($(this).attr('fechafin'));
