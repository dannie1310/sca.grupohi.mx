...
<!--<table class="reporte" align="center" style="width:800px; display:none;">
<?php
...
while($vmateriales = $SCA->fetch($rmateriales)){
	...
	while($vbloques = $SCA->fetch($rbloques)){
		...
		while($vtopografias = $SCA->fetch($rtopografias)){
            ...
            <tr style="display:none;" id="topografia<?php echo $vbloques['IdBloque'];?><?php echo $vmateriales['IdMaterial'];?>" class="topografia<?php echo $vmateriales['IdMaterial'];?>">
            	<td>&nbsp;</td>
            	<td>&nbsp;</td>
