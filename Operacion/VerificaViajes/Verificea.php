...
session_start();
	...
	$r=$l->consultar($SQLs);
	$condicion="";
	function muestra_viajes($condicion_t)
	{
		$respuesta=new xajaxResponse();
		$condicion.=$condicion_t;
		$pagina=file_get_contents("http://192.168.103.154/SCACLT/Operacion/VerificaViajes/VerificaFormulario.php?idp=".$_SESSION["Proyecto"]."&condicion=".$condicion_f);
		$respuesta->assign("contenido","innerHTML",$pagina);
		return $respuesta;
	}
	...
	$xajax->register(XAJAX_FUNCTION,"calculos_x_tipo_tarifa");
		$xajax->register(XAJAX_FUNCTION,"muestra_viajes");
...
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
 ...
 ?>
<title>Validaci&oacute;n ed Viajes</title>
...
<body onload="carga_load('../../Imagenes/loading.gif','Cargando...')">
<div id="layout">
...
<table style="width:90%;margin-left:5%"border="0" cellspacing="0" cellpadding="2">
<?php 
$i_general=1;
$i_padre=1;
//echo ''.$l->affected;
while($v=$l->fetch($r)) { ?>
  ...
  <tr  style="display:none" id="<?php echo $v["FechaO"]?>">
    <td><table style="width:95%;margin-left:2.5%"border="0" cellspacing="0" cellpadding="2">
    <?php
	...
	while($v_camiones=$l->fetch($r_camiones)) { 
		...
		var Arreglo_Procesar_<?php echo $i_padre ?>=new Array();
          ...
          <tr style="display:none" id="<?php echo $v["FechaO"].$v_tiros["IdTiro"].$v_camiones["Camion"]?>">
            <td><table border="0" cellpadding="2" cellspacing="0" class="formulario"  style="width:auto">
              <tr>
                <td align="center">&nbsp;</td>
                <td align="center">&nbsp;</td>
                <td align="center">&nbsp;</td>
              ...
              <tr class="Item">
                <td align="center">#</td>
                <td align="center">?</td>
                <td align="center"><img src="../../Imagenes/ok.gif" alt="" width="16" height="16" /></td>
                <td align="center"><img src="../../Imagenes/ko.gif" alt="" width="16" height="16" /></td>
                <td align="center">Cubicaci&oacute;n</td>
                <td align="center">Ori�gen<br />
				<?php $lista_origenes_padre=$l->regresaSelect_evt("origen_padre".$i_padre,"distinct(origenes.IdOrigen), concat(origenes.Clave,'-',origenes.IdOrigen) AS Clave, origenes.Descripcion","origenes join rutas on (rutas.IdOrigen=origenes.IdOrigen AND rutas.Estatus=1) ","origenes.IdProyecto = ".$_SESSION["Proyecto"]." AND origenes.Estatus = 1","IdOrigen","Descripcion","desc","100px","1","0","1"," onChange='cambia_origen_child(Arreglo_".$i_padre.",\"".$i_padre."\")'","","r");  echo $lista_origenes_padre;?></td>
                <td align="center">Material</td>
                <td align="center">Tiempo</td>
                <td align="center">Ruta</td>
                <td align="center">Distancia</td>
                <td align="center">Importe</td>
                <td align="center">Tipo de<br />Tarifa</td>
                <td align="center" style="display:none">Horas Efectivas</td>
                <td align="center" style="display:none">Cargador</td>
                </tr>
                <?php 
				$SQLs = "
				select
...
if(r.TotalKM>=30,2.20*r.KmSubsecuentes,tr.KMSubsecuente*r.KmSubsecuentes) as ImporteKS_R,
tr.KMAdicional*r.KmAdicionales as ImporteKA_R,
if(r.TotalKM>=30,((4.40)+(2.20*r.KmSubsecuentes)+(tr.KMAdicional*r.KmAdicionales)),((tr.PrimerKM*1)+(tr.KMSubsecuente*r.KmSubsecuentes)+(tr.KMAdicional*r.KmAdicionales))) as ImporteTotal_R,
tm.PrimerKM*1 as ImportePK_M,
tm.KMSubsecuente*r.KmSubsecuentes as ImporteKS_M,
tm.KMAdicional*r.KmAdicionales as ImporteKA_M,
((tm.PrimerKM*1)+(tm.KMSubsecuente*r.KmSubsecuentes)+(tm.KMAdicional*r.KmAdicionales)) as ImporteTotal_M
from
...
<input name="idviaje<?php echo $i_general; ?>" id="idviaje<?php echo $i_general; ?>" type="hidden" value="<?php echo $v_viajes["IdViaje"]; ?>" />
             <tr>
                ...
                <td><div id="imagen<?php echo $i_general; ?>"><img src="../../Imagenes/<?php if($v_viajes["idruta"]==''||$v_viajes["tarifa_ruta"]==''||$v_viajes["Estatus"]==10||($v_viajes["Estatus"]==0&&($v_viajes["tiempo"]==0||($v_viajes["tiempo"]>$v_viajes["cronometria"])||($v_viajes["tiempo"]<$v_viajes["TiempoMinimo"])))) echo "bred"; else echo "bgreen"; ?>.gif" <?php
				...
				if($v_viajes["tiempo"]!=0&&$v_viajes["Estatus"]==0&&(($v_viajes["tiempo"]>$v_viajes["cronometria"])||($v_viajes["tiempo"]<$v_viajes["TiempoMinimo"]))) {echo "title='El viaje no puede ser registrado porque no cumple con los tiempos de cronometr&iacute;a de la ruta'";} 
				else
				...
				else if($v_viajes["tarifa_ruta"]==''&&$v_viajes["idruta"]!=''&&$v_viajes["Estatus"]==0) echo "title='El viaje no puede ser registrado porque no hay una tarifa registrada para su ruta'";
                ...
                <td align="center"><div id="ch<?php echo $i_general ?>">
                  ...
                  onclick="clic('r<?php echo $i_general; ?>')" <?php if($v_viajes["idruta"]==''||$v_viajes["tarifa_ruta"]==''||$v_viajes["Estatus"]==10||($v_viajes["Estatus"]==0&&($v_viajes["tiempo"]==0||($v_viajes["tiempo"]>$v_viajes["cronometria"])||($v_viajes["tiempo"]<$v_viajes["TiempoMinimo"])))) echo "disabled"; else echo "checked"; ?> style="cursor:pointer"/>
               </div></td>
                <td align="center"><div id="nch<?php echo $i_general ?>">
                  <input type="checkbox" name="r<?php echo $i_general; ?>" id="r<?php echo $i_general; ?>"  <?php if($v_viajes["idruta"]==''||$v_viajes["tarifa_ruta"]==''||$v_viajes["Estatus"]==10||($v_viajes["Estatus"]==0&&($v_viajes["tiempo"]==0||($v_viajes["tiempo"]>$v_viajes["cronometria"])||($v_viajes["tiempo"]<$v_viajes["TiempoMinimo"])))){ echo "checked"; }?> onclick="clic('a<?php echo $i_general; ?>')" style="cursor:pointer"/>
                </div></td>
                <td align="right" class="detalle"><?php echo $v_viajes["cubicacion"] ?>m<sup>3</sup></td>
                <td align="center"><?php if($v_viajes["origen"]!='') {?><span class="detalle"><?php echo $v_viajes["origen"] ; ?><input name="<?php echo "origen".$i_general;?>" id="<?php echo "origen".$i_general;?>" type="hidden" value="<?php echo $v_viajes["idorigen"]; ?>" /></span><?php }else {
				
				$lista_origenes=$l->regresaSelect_evt("origen".$i_general,"distinct(origenes.IdOrigen), concat(origenes.Clave,'-',origenes.IdOrigen) AS Clave, origenes.Descripcion","origenes join rutas on (rutas.IdOrigen=origenes.IdOrigen AND rutas.Estatus=1) ","origenes.IdProyecto = ".$_SESSION["Proyecto"]." AND origenes.Estatus = 1","IdOrigen","Descripcion","desc","100px","1","0","1"," onChange='xajax_calculos_x_tipo_tarifa(document.getElementById(\"tarifa".$i_general."\").value,\"".$v_viajes["idmaterial"]."\",\"".$i_general."\",document.getElementById(\"origen".$i_general."\").value,document.getElementById(\"tiro".$i_general."\").value,document.getElementById(\"camion".$i_general."\").value,1)'","","r");  echo $lista_origenes;?>
                <script>
        Arreglo_<?php echo $i_padre ?>.push('<?php echo $i_general; ?>');
                ...
                <td align="center"><span class="detalle">
                  ...
                  <?php echo $v_viajes["tiempo"] ?></span></td>
                ...
                <td align="center"><span class="detalle"><div id="imp<?php echo $i_general; ?>"><?php echo $v_viajes["ImporteTotal_R"] ?>
                ...
                <td align="center"><select name="tarifa<?php echo $i_general; ?>" id="tarifa<?php echo $i_general; ?>" onchange="xajax_calculos_x_tipo_tarifa(this.value,'<?php echo $v_viajes["idmaterial"]; ?>','<?php echo $i_general; ?>',document.getElementById('origen<?php echo $i_general; ?>').value,document.getElementById('tiro<?php echo $i_general; ?>').value,document.getElementById('camion<?php echo $i_general; ?>').value,0)">
              ...
              <?php $i_general++; $i++;}?>
                  ...
                  <td align="right">&nbsp;</td>
                  <td align="right">&nbsp;</td>
                  <td align="center"><input name="button" type="submit" class="boton" id="button" value="Registrar Viajes" onclick="prepara_registra_viajes(Arreglo_Procesar_<?php echo $i_padre ?>)"/></td>
                  <td align="right" style="display:none">&nbsp;</td> 
                  <td align="center" style="display:none">&nbsp;</td>
                  </tr>
            </table></td>
...
function cambia_origen_child(arreglo,i_padre)
{
	...
	{
		xajax_calculos_x_tipo_tarifa(document.getElementById("tarifa"+arreglo[o]).value,document.getElementById("material"+arreglo[o]).value,arreglo[o],document.getElementById("origen_padre"+i_padre).value,document.getElementById("tiro"+arreglo[o]).value,document.getElementById("camion"+arreglo[o]).value,1);
...
for(o=0;o<arreglo.length;o++)
	{
		...
		a=document.getElementById('a'+arreglo[o]).checked;
		r=document.getElementById('r'+arreglo[o]).checked;
		//alert(id_viaje_neto+''+a+' '+r);
		//alert(accion);
		if(accion!='n')
		xajax_registra_viaje(arreglo[o],accion,id_viaje_neto,maquinaria,horas_efectivas,origen,tarifa);
