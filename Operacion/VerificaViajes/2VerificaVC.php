...
<table width="845" border="0" cellpadding="0" cellspacing="0" align="center" bordercolor="#FFFFFF">
  <tr>
    <td class="EncabezadoPagina"><img src="../../Imgs/Logos/Operacion/ValidaViajes.gif" alt="" width="24" height="24" align="absbottom" />SCA.- Validaci&oacute;n de Viajes Con Or&iacute;gen</td>
  </tr>
  <tr>
    <td>&nbsp;</td>
  </tr>
...
<?php
	...
	//Obtenemos las Fechas de los Viajes Pendientes de Validar
		...
		$SQL="SELECT  viajesnetos.FechaLlegada as Fecha,COUNT(viajesnetos.IdViajeNeto) AS Total FROM viajesnetos WHERE viajesnetos.Estatus = 0 AND viajesnetos.IdProyecto = ".$_SESSION['Proyecto']." GROUP BY viajesnetos.FechaLlegada;";
		//echo "<br><br>".$SQL;
		...
		while($Row=mysql_fetch_array($Result))
		 ...
		 //Nos Traemos los Tiros Trabajados para Cada una de las Fechas
			$Contador=0;
  			...
  			{
			 	$Link=SCA::getConexion();
	  			$SQL="SELECT tiros.IdTiro,tiros.Descripcion, COUNT(viajesnetos.IdTiro) AS Total FROM viajesnetos, tiros WHERE viajesnetos.IdTiro = tiros.IdTiro AND viajesnetos.FechaLlegada = '".$FechasValidas[$a][1]."' AND viajesnetos.IdProyecto = ".$_SESSION['Proyecto']." AND viajesnetos.Estatus = 0 GROUP BY tiros.Descripcion";
	  			//echo "<BR><BR>Consulta para obtener los Tiros por Fecha".$SQL;
	  			$Result=$Link->consultar($SQL);
	  			$Link->cerrar();

	  			//Guardamos los datos de los Tiros en un Arreglo
	  			while($Row=mysql_fetch_array($Result))
	  			{
			...
			//echo "<br>Total de Tiros: ".count($TirosPorFecha);
  			...
  			//Nos traemos los Camiones por Tiro y para la Fecha Correspondiente
				$Contador=0;
  				...
  				{
          			$Link=SCA::getConexion();
					$SQL="SELECT viajesnetos.IdCamion, camiones.Economico, count(camiones.Economico) AS Total FROM viajesnetos, camiones WHERE viajesnetos.IdProyecto=".$_SESSION['Proyecto']." AND viajesnetos.FechaLlegada = '".$TirosPorFecha[$c][1]."' AND viajesnetos.IdTiro = ".$TirosPorFecha[$c][4]." AND viajesnetos.Estatus = 0 AND viajesnetos.IdCamion = camiones.IdCamion GROUP BY camiones.Economico";
				...
				//Obtenemos los Datos de los Viajes por Camion, Tiro, y fecha
					...
					{
						$Link=SCA::getConexion();
				  		$SQL="SELECT camiones.Economico AS Economico, materiales.Descripcion AS Material, materiales.IdMaterial, origenes.Descripcion AS Origen,ROUND((HOUR(TIMEDIFF(viajesnetos.HoraLlegada,viajesnetos.HoraSalida))*60)+(MINUTE(TIMEDIFF(viajesnetos.HoraLlegada,viajesnetos.HoraSalida)))+(SECOND(TIMEDIFF(viajesnetos.HoraLlegada,viajesnetos.HoraSalida))/60),2) AS TiempoViajeMinutos,origenes.IdOrigen, camiones.IdCamion, viajesnetos.IdViajeNeto,camiones.CubicacionParaPago FROM viajesnetos, camiones, tiros, materiales, origenes WHERE camiones.IdCamion = viajesnetos.IdCamion AND tiros.IdTiro = viajesnetos.IdTiro AND viajesnetos.IdMaterial = materiales.IdMaterial AND viajesnetos.IdOrigen = origenes.IdOrigen AND viajesnetos.IdProyecto=".$_SESSION['Proyecto']." AND viajesnetos.FechaLlegada = '".$CamionesPorFechaYTiro[$d][1]."' AND viajesnetos.IdTiro=".$CamionesPorFechaYTiro[$d][2]." AND viajesnetos.IdCamion=".$CamionesPorFechaYTiro[$d][3]."  AND viajesnetos.Estatus = 0  ORDER BY camiones.Economico,viajesnetos.FechaLlegada, viajesnetos.HoraLlegada;";
						...
						while ($Row=mysql_fetch_array($Result))
				  		{
				  			...
				  			$ViajesPorTiro[$Contador][2]=$CamionesPorFechaYTiro[$d][2];			//Id del Tiro del Viaje
				  			...
				  			$ViajesPorTiro[$Contador][9]=RegresaRutaViaje($_SESSION['Proyecto'],$ViajesPorTiro[$Contador][7],$CamionesPorFechaYTiro[$d][2]);		//Id de la Ruta del Viaje
...
<table width="845" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
    <td colspan="5"><table id="fechas<?php  echo $a;?>"  >
      <?php
	  	//Mostramos los Tiros Trabajados en esa Fecha
      ...
      <tr>
        <td colspan="5">
        <table id="f<?php echo $a;  ?>t<?php echo $b;?>" >
              <form name="frm<?php echo $b; ?>" method="post" action="3VerificaVC.php">
        <?php
        ...
        <tr id="f<?php echo $a;  ?>t<?php echo $b;  ?>c<?php echo $c; ?>" >
          <td colspan="5"><table>
          ...
          <td ><table  width="731" border="1" cellpadding="0" cellspacing="0" bordercolor="#FFFFFF">
                              ...
                              <td colspan="10" >&nbsp;</td>
                               ...
                               <td class="Item1">&nbsp;
                              <?php combo_cargador_padre("maquinaria","Economico","IdMaquinaria",$a.'_'.$b.'_'.$c,$CamionesPorFechaYTiro[$c][5]);
							  ?></td>
                            </tr>
            <tr>
              <td>&nbsp;</td>
            ...
            <tr id="fil<?php echo $ViajesPorTiro[$d][14]; ?>">
              ...
              <?php
													...
													if($ViajesPorTiro[$c][13]==1)
							 						{ echo'<td class="Item1"><img src="../../Imgs/16-True.gif" width="16" height="16"></td>'; }
							 						else
              ...
              <td class="Item1" valign="top" align="right">&nbsp;<?php echo $ViajesPorTiro[$d][16]; ?>&nbsp;
                      <input type="hidden" name="Tiempo<?php echo $ph; ?>" value="<?php echo $ViajesPorTiro[$d][6]; ?>" />
                      <input type="hidden" name="Ruta<?php echo $ph; ?>" value="<?php echo $ViajesPorTiro[$d][9]; ?>" />
                      <input type="hidden" name="Distancia<?php echo $ph; ?>" value="<?php echo $ViajesPorTiro[$d][10]; ?>" />
                      <input type="hidden" name="Importe<?php echo $ph; ?>" value="<?php echo $ViajesPorTiro[$c][16]; ?>" />
                      <input type="hidden" name="Cubicacion<?php echo $ph; ?>" value="<?php echo $ViajesPorTiro[$d][15]; ?>" />
                      <input type="hidden" name="ImportePrimerKM<?php echo $ph; ?>" value="<?php echo $ViajesPorTiro[$d][27]; ?>" />
                      <input type="hidden" name="ImporteKMSub<?php echo $ph; ?>" value="<?php echo $ViajesPorTiro[$d][28]; ?>" />
                      <input type="hidden" name="ImporteKMAdc<?php echo $ph; ?>" value="<?php echo $ViajesPorTiro[$d][29]; ?>" />
                      <input type="hidden" name="VolumenPrimerKM<?php echo $ph; ?>" value="<?php echo $ViajesPorTiro[$d][23]; ?>" />
                      <input type="hidden" name="VolumenKMSub<?php echo $ph; ?>" value="<?php echo $ViajesPorTiro[$d][24]; ?>" />
                      <input type="hidden" name="VolumenKMAdc<?php echo $ph; ?>" value="<?php echo $ViajesPorTiro[$d][25]; ?>" />
                      <input type="hidden" name="Volumen<?php echo $ph; ?>" value="<?php echo $ViajesPorTiro[$d][26]; ?>" />              </td>
                      <td class="Item1"><?php //combo_cargador("maquinaria","Economico","IdMaquinaria",$ph,"h") 
					  combo_cargador_hijo("maquinaria","Economico","IdMaquinaria",$a.'-'.$b.'-'.$c,$ph);
					  ?></td>
                       <td class="Item1"><input name="hef<?php echo $ph; ?>" type="text" class="Casillas_small" size="5" maxlength="7" onkeypress="onlyDigits(event,'decOK')"  /></td>
