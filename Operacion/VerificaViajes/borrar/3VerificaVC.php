...
$FactorAbundamiento=regresa_factor($Viajes[$a][13]);
			$Link=SCA::getConexion();
			$SQL="insert into viajes(HorasEfectivas,IdMaquinaria,FactorAbundamiento,IdViajeNeto,FechaCarga,HoraCarga,IdProyecto,Creo,TiempoViaje,IdRuta,Distancia,VolumenPrimerKM,VolumenKMSubsecuentes,VolumenKMAdicionales,Volumen,ImportePrimerKM,ImporteKMSubsecuentes,ImporteKMAdicionales,Importe,IdCamion,CubicacionCamion,IdOrigen,FechaSalida,HoraSalida,IdTiro,FechaLlegada,HoraLlegada,IdMaterial) values(".$Viajes[$a][28].",".$Viajes[$a][27].",".$FactorAbundamiento.",".$Viajes[$a][1].",'".$Hoy."','".$Hora."',".$_SESSION['Proyecto'].",'".$Creo."',".$Viajes[$a][2].",".$Viajes[$a][3].",".$Viajes[$a][4].",'".$Viajes[$a][17]."','".$Viajes[$a][18]."','".$Viajes[$a][19]."',".$Viajes[$a][20].",".$Viajes[$a][14].",".$Viajes[$a][15].",".$Viajes[$a][16].",'".$Viajes[$a][5]."',".$Viajes[$a][6].",".$Viajes[$a][21].",".$Viajes[$a][7].",'".$Viajes[$a][8]."','".$Viajes[$a][9]."',".$Viajes[$a][10].",'".$Viajes[$a][11]."','".$Viajes[$a][12]."',".$Viajes[$a][13].");";
			//echo "<br><br>".$SQL;
			$Result=$Link->consultar($SQL);
			$afe=$Link->affected();
			if($afe==1)
			$TotalInsertados=$TotalInsertados+$afe;
			//echo "<br>afe: ".$afe;
			//echo "<br>TotalInsertados: ".$TotalInsertados;
			...
			//echo 'af'.$afe;
				...
				$SQL="update viajesnetos set Estatus=1 where IdViajeNeto=".$Viajes[$a][1].";";
				//echo "<br>".$SQL;
			...
			{
				if($TotalAProcesar==1)
				$error[$e]="HUBO UN ERROR AL PROCESAR EL VIAJE ".$Viajes[$a][1].", VERIFIQUE QUE EXISTA UNA RUTA PARA ESE VIAJE";
				else
				$error[$e]="HUBO UN ERROR AL PROCESAR EL VIAJE ".$Viajes[$a][1].", VERIFIQUE QUE EXISTA UNA RUTA PARA ESE VIAJE";
		...
		$e=0;
		$TotalRechazadosAProcesarError=0;
		...
		for($a=0;$a<$TotalRechazadosAProcesar;$a++)
		{
           ...
           $FactorAbundamiento=regresa_factor($ViajesRechazados[$a][13]);
			$Link=SCA::getConexion();
			$SQL="insert into viajesrechazados(HorasEfectivas,FactorAbundamiento,IdViajeNeto,FechaRechazo,HoraRechazo,IdProyecto,Creo,TiempoViaje,IdRuta,Distancia,VolumenPrimerKM,VolumenKMSubsecuentes,VolumenKMAdicionales,Volumen,ImportePrimerKM,ImporteKMSubsecuentes,ImporteKMAdicionales,Importe,IdCamion,CubicacionCamion,IdOrigen,FechaSalida,HoraSalida,IdTiro,FechaLlegada,HoraLlegada,IdMaterial) values(".$ViajesRechazados[$a][28].",".$ViajesRechazados[$a][27].",".$FactorAbundamiento.",".$ViajesRechazados[$a][1].",'".$Hoy."','".$Hora."',".$_SESSION['Proyecto'].",'".$Creo."',".$ViajesRechazados[$a][2].",'".$ViajesRechazados[$a][3]."','".$ViajesRechazados[$a][4]."',".$ViajesRechazados[$a][17].",".$ViajesRechazados[$a][18].",".$ViajesRechazados[$a][19].",".$ViajesRechazados[$a][20].",'".$ViajesRechazados[$a][14]."','".$ViajesRechazados[$a][15]."','".$ViajesRechazados[$a][16]."','".$ViajesRechazados[$a][5]."',".$ViajesRechazados[$a][6].",".$ViajesRechazados[$a][21].",".$ViajesRechazados[$a][7].",'".$ViajesRechazados[$a][8]."','".$ViajesRechazados[$a][9]."',".$ViajesRechazados[$a][10].",'".$ViajesRechazados[$a][11]."','".$ViajesRechazados[$a][12]."',".$ViajesRechazados[$a][13].");";
			//echo "<br>".$SQL;
			$Result=$Link->consultar($SQL);
			$afe=$Link->affected();
			if($afe==1)
			$TotalRechazados=$TotalRechazados+$afe;
			...
			if($afe==1)
			{
				//Actualizamos el Estatus de los Viajes Netos Procesados
				$Link=SCA::getConexion();
				$SQL="update viajesnetos set Estatus=2 where IdViajeNeto=".$ViajesRechazados[$a][1].";";
				//echo "<br>".$SQL;
				$Result=$Link->consultar($SQL);
				$Link->cerrar();
			}
			...
			{
				if($TotalRechazadosAProcesar==1)
				$error[$e]="HUBO UN ERROR AL PROCESAR EL VIAJE ".$ViajesRechazados[$a][1].", VERIFIQUE QUE EXISTA UNA RUTA PARA ESE VIAJE";
				else
				$error[$e]="HUBO UN ERROR AL PROCESAR EL VIAJE ".$ViajesRechazados[$a][1].", VERIFIQUE QUE EXISTA UNA RUTA PARA ESE VIAJE";
...
<?php
		...
		//for($e2=0;$e2<count($error);$e2++)
	//	{?>
        <tr class="TituloDeAdvertencia">
			<td>
				<?php echo $error[$e2].'<br>'; ?>
            </td>
        </tr>
<?php
		//}?>
		...
		echo "<br><br>Usted Ha Rechazado ".$TotalRechazados." Viajes Completos en el Sistema . . . <br><br>";
		if($TotalRechazados>=1)
		...
		{
			$Link=SCA::getConexion();
			$SQL="SELECT origenes.Descripcion AS Origen,tiros.Descripcion AS Tiro,camiones.Economico,materiales.Descripcion as Material FROM viajesnetos,materiales,origenes,tiros, camiones WHERE materiales.IdMaterial = viajesnetos.IdMaterial AND origenes.IdOrigen = viajesnetos.IdOrigen AND tiros.IdTiro = viajesnetos.IdTiro AND camiones.IdCamion = viajesnetos.IdCamion  AND viajesnetos.IdViajeNeto = ".$viajeRechazarError[$a][1]."  AND viajesnetos.IdTiro=".$viajeRechazarError[$a][10]." AND viajesnetos.IdOrigen=".$viajeRechazarError[$a][7]." ORDER BY viajesnetos.FechaLlegada,viajesnetos.HoraLlegada";
			//echo "<br>".$SQL;
			$Result=$Link->consultar($SQL);
			$Link->cerrar();
