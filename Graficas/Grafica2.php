...
//$anio = $_POST['anio'];
$anio = 2011;
$sql1 = "SELECT SUM(v.importe/1000) AS ImpTotal,v.IdMaterial,m.Descripcion
						FROM viajes AS v
		...
		<script type="text/javascript">
			...
			$(document).ready(function() {
				chart = new Highcharts.Chart({
					...
					title: {
						text: 'Importe Total $<?php echo $english_format_number2 = number_format($Sumatoria1, 2); ?>'
					},
					subtitle: {
						text: ''
					},
					xAxis: {
						categories: [<?php $material = "SELECT SUM(v.importe/1000) AS ImpTotal,v.IdMaterial,m.Descripcion
FROM viajes AS v
LEFT JOIN materiales AS m ON v.IdMaterial = m.IdMaterial
WHERE YEAR(fechallegada)=$anio
GROUP BY v.IdMaterial ORDER BY ImpTotal DESC";
					...
					credits: {
				        ...
				        series: [{
						name: 'Importe Total',
						data: [<?php $volumen = "SELECT SUM(v.importe/1000) AS ImpTotal,v.IdMaterial,m.Descripcion
FROM viajes AS v
LEFT JOIN materiales AS m ON v.IdMaterial = m.IdMaterial
WHERE YEAR(fechallegada)=$anio
GROUP BY v.IdMaterial ORDER BY ImpTotal DESC";
...
<table>
        	<tr>
            	<td class="EncabezadoPagina1">
                	Grafica Importes Totales Por Material del <?php echo $anio; ?>
                </td>
            </tr>
        </table>
		<div id="container" style="width: 950px; height: 900px; display:inline-block;"></div>
