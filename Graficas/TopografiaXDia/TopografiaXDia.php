...
$qvalida = "select distinct DATE_FORMAT(Fecha,'%d-%m-%Y') as Dia from topografias where Fecha>='".$inicial."' AND Fecha<='".$final."' order by Fecha";
$rvalida = $SCA->consultar($qvalida);
$nvalida = $SCA->affected();
if($nvalida>0){
?>
<!DOCTYPE HTML>
<html>
	<head>
		...
		<title>Topografia Por Dia .- SCA</title>
...
$(function () {
    ...
    $(document).ready(function() {
		...
		subtitle: {
                text: 'DEL: <?php echo $_REQUEST['inicial'].' AL: '.$_REQUEST['final'];?>'
            },
		credits: {
            text: 'SCA Zapotillo',
        },
            ...
            xAxis: {
				...
				categories: [
							 <?php
							 $qfechas = "select distinct DATE_FORMAT(Fecha,'%d-%m-%Y') as Dia from topografias where Fecha>='".$inicial."' AND Fecha<='".$final."' order by Fecha";
			...
			crosshairs: true,
                ...
                formatter: function() {
                    var s;
                        ...
                        s = '<strong>Fecha: </strong>'+this.x  +'<br /><strong>Material: </strong>'+this.series.name+'<strong><br /><strong>Volumen: </strong>'+ Highcharts.numberFormat(this.y, 2)+'m<sup>3</sup></strong>';
			...
			<?php
			$qmateriales = "select distinct(t.IdMaterial) as IdMaterial,m.Descripcion from topografias as t left join materiales as m on m.IdMaterial = t.IdMaterial where Fecha>='".$inicial."' AND Fecha<='".$final."' order by IdMaterial";
			...
			while($vmateriales=$SCA->fetch($rmateriales)){
				?>
				{
					...
					data: [
						   <?php
						   $qfechas = "select distinct Fecha from topografias where Fecha>='".$inicial."' AND Fecha<='".$final."' order by Fecha";
						   $rfechas = $SCA->consultar($qfechas);
						   $nfechas = $SCA->affected();
						   $m=0;
						   while($vfechas = $SCA->fetch($rfechas)){
...
<div id="container" style="border:#F00 solid 2px; padding:10px; width:600px; background-color:#FFE6E6; color:#F00; margin-left:500px; font-size:14px;">No hay datos por mostrar, seleccione otro rango de fecha</div>
</body>
    <?php
	}
}else{
