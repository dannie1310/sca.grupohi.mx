...
$SQL="SELECT DISTINCT viajes.FechaSalida, weekday(viajes.FechaSalida) AS Dia FROM viajes WHERE viajes.IdProyecto = ".$_SESSION["Proyecto"]." AND weekofyear(viajes.FechaSalida) = ".$Semana." AND year(viajes.FechaSalida) = ".$Anio." ORDER BY viajes.FechaSalida;";
//echo "<br><br>".$SQL;
...
//Llenamos los Datos del Reporte
	...
	//Nos Traemos los Datos Necesarios
		...
		//Traemos los Tiros Involucrdos para la semana en cuestion
			$Link=SCA::getConexion();
			//$SQL="SELECT tiros.Descripcion,viajes.IdTiro FROM viajes, tiros WHERE viajes.IdTiro = tiros.IdTiro AND viajes.IdProyecto = ".$_SESSION["Proyecto"]." AND weekofyear(viajes.FechaSalida) = ".$Semana." AND year(viajes.FechaSalida)=".$Anio." GROUP BY tiros.Descripcion;";
			//echo "<br><br>".$SQL;
			$Result=$Link->consultar($SQL);
			$Link->cerrar();
		...
		//Nos Traemos los Camiones que participaron en el Tiro
			...
			{
				$Link=SCA::getConexion();
				$SQL="SELECT DISTINCT viajes.IdCamion, camiones.Economico, camiones.CubicacionParaPago FROM viajes, camiones WHERE camiones.IdCamion = viajes.IdCamion AND viajes.IdProyecto = ".$_SESSION["Proyecto"]." AND viajes.FechaSalida IN ('".$DiasDeLaSemana[0]."','".$DiasDeLaSemana[1]."','".$DiasDeLaSemana[2]."','".$DiasDeLaSemana[3]."','".$DiasDeLaSemana[4]."','".$DiasDeLaSemana[5]."','".$DiasDeLaSemana[6]."') AND viajes.IdTiro = ".$Tiros[$a][2]." AND year(viajes.FechaSalida)=".$Anio." ORDER BY camiones.Economico;";
				//echo "<br><br>".$SQL;
				$Result=$Link->consultar($SQL);
				$Link->cerrar();
				...
				while($Row=mysql_fetch_array($Result))
				{
					...
					$ConcentradoViajes[$Contador][4]=RegresaTotalViajesXDiaXTiro($_SESSION["Proyecto"],$DiasDeLaSemana[0],$Tiros[$a][2],$ConcentradoViajes[$Contador][1]);		//Total de Viajes para la Fecha 1 de la Semana
					$ConcentradoViajes[$Contador][5]=RegresaTotalViajesXDiaXTiro($_SESSION["Proyecto"],$DiasDeLaSemana[1],$Tiros[$a][2],$ConcentradoViajes[$Contador][1]);		//Total de Viajes para la Fecha 2de la Semana
					$ConcentradoViajes[$Contador][6]=RegresaTotalViajesXDiaXTiro($_SESSION["Proyecto"],$DiasDeLaSemana[2],$Tiros[$a][2],$ConcentradoViajes[$Contador][1]);		//Total de Viajes para la Fecha 3 de la Semana
					$ConcentradoViajes[$Contador][7]=RegresaTotalViajesXDiaXTiro($_SESSION["Proyecto"],$DiasDeLaSemana[3],$Tiros[$a][2],$ConcentradoViajes[$Contador][1]);		//Total de Viajes para la Fecha 4 de la Semana
					$ConcentradoViajes[$Contador][8]=RegresaTotalViajesXDiaXTiro($_SESSION["Proyecto"],$DiasDeLaSemana[4],$Tiros[$a][2],$ConcentradoViajes[$Contador][1]);		//Total de Viajes para la Fecha 5 de la Semana
					$ConcentradoViajes[$Contador][9]=RegresaTotalViajesXDiaXTiro($_SESSION["Proyecto"],$DiasDeLaSemana[5],$Tiros[$a][2],$ConcentradoViajes[$Contador][1]);		//Total de Viajes para la Fecha 6 de la Semana
					$ConcentradoViajes[$Contador][10]=RegresaTotalViajesXDiaXTiro($_SESSION["Proyecto"],$DiasDeLaSemana[6],$Tiros[$a][2],$ConcentradoViajes[$Contador][1]);		//Total de Viajes para la Fecha 7 de la Semana
					...
					$ConcentradoViajes[$Contador][13]=RegresaImporteSumaViajesXTiroXFecha($_SESSION["Proyecto"],$Semana,$Tiros[$a][2],$ConcentradoViajes[$Contador][1]);	//Importe Sin Formato
