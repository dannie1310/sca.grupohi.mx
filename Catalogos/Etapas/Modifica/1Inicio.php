...
<table width="333" border="0" align="center">
  ...
  <tr>
    ...
    <td width="543" class="EncabezadoTabla">DESCRIPCI&Oacute;N</td>
    <td width="89" class="EncabezadoTabla">ESTATUS</td>
    </tr>
 
  <?php
  $sql="select Descripcion,IdEtapaProyecto,case Estatus
  when 0 then 'INACTIVO'
  when 1 then 'ACTIVO'
  end Estate FROM etapasproyectos ;";
	//echo $sql;
	$pr=1;
  $result=$link->consultar($sql);
  while($row=mysql_fetch_array($result))
  ...
  <td><input name="radiobutton" type="radio" value="radiobutton" onClick="document.frm.etapa.value=<?php echo $row[IdEtapaProyecto]; ?>;valida();">      </td>
