...
<?php
  ...
  $obtiene="select IdCentroCosto, Nivel,  length(Nivel) as niv,concat(repeat('&nbsp;&nbsp;&nbsp;',(length(Nivel)/4)),'=> ',Descripcion)as Descripcion FROM centroscosto WHERE IdProyecto= ".$_SESSION[Proyecto]." ORDER BY Nivel";
...
<tr>
  <td class="textoG"> REGISTRAR CENTRO DE COSTO NIVEL 0 </td>
...
<tr id="r0" <?php if($padre!=0||$padre==''){?>style="display:none " <?php } ?>>
  <td><input name="descripcion0" id="descripcion0" <?php if($padre==0){?>value="<?php echo $descripcion; ?>"<?php }?> type="text" class="texto" size="50" maxlength="50" style="text-transform:uppercase" onkeypress="withoutSpaces(event,'decOK')" onKeyUp="val(<?php echo $pr;?>)"></td>
  <td align="center">
  	<input id="envia<?php echo $pr;?>"  name="envia<?php echo $pr;?>" type="button" onClick="document.frm.descripcion.value=document.frm.descripcion<?php echo $pr; ?>.value;document.frm.padre.value='0';this.form.submit()" class="boton" value="Registrar" disabled>
  </td>
</tr>
<tr>
  <td colspan="2">&nbsp;</td>
  ...
  <tr>
    <td width="320" class="EncabezadoTabla"><input type="hidden" name="descripcion">
	<input type="hidden" name="padre">
      DESCRIPCI&Oacute;N</td>
    <td width="70" class="EncabezadoTabla">&nbsp;</td>
  </tr>
  <?PHP
  ...
  <tr <?php if($v[IdCentroCosto]!=$padre){?>style="display:none " <?php } ?> id="r<?php echo $pr;?>" class=" <?php $a=$pr%2; if($a==0){ ?> Item2 <?php } else {?> Item1 <?php } ?>">
    ...
    <td>
	<input name="envia<?php echo $pr;?>" id="envia<?php echo $pr;?>" type="button" onClick="document.frm.descripcion.value=document.frm.descripcion<?php echo $pr; ?>.value;document.frm.padre.value=document.frm.padre<?php echo $pr; ?>.value;this.form.submit()" class="boton" value="Registrar" disabled>
