...
<?php }
	else
	{
			...
			//Abrimos el Archivo que se Acaba de Subir
								...
								$ContenidoArchivo = fread($AbrirArchivo, filesize($NombreArchivo));
								...
								if($p_len==4&&trim($primera_linea[3])=="T1"&&trim($ultima_linea)=="T 000")
								{
									//copy($_FILES["archivo_botones"]["tmp_name"],$Ruta.$NombreArchivo);
										...
										for($o=1;$o<sizeof($ContenidoArchivoExplotado)-2;$o++)
										{
											...
											if($v2["@Respuesta"]!='')
											{
												...
												if($mensaje[0]=='ok')
												$registrados++;
												$respuesta[$o]='<img src="../../../../Imagenes/'.$mensaje[0].'.gif" width="16" height="16" align="absbottom" /> '.utf8_encode($mensaje[1]);
											}
											else
											{
												$respuesta[$o]="<img src='../../../../Imagenes/ko.gif' width='16' height='16' align='absbottom' /> Hubo un error durante el registro del botón: ".$identificador[$o];	
												$no_registrados++;
											}
										}?>
